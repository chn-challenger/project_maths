...
describe Expression do
  ...
  describe '#==' do
    ...
    it 'returns false when compared expression is not an expression' do |_variable|
      expression1 = Expression.new([Step.new(:add, 5), Step.new(:mtp, 'x')])
  ...
  describe '#expand' do
    ...
    context 'with left steps' do
      ...
      it 'expands two brackets which has left steps' do
        ...
        expected_exp = msum_factory.build([[nil, ['x', 4]], [:add, [3, 4]], [:sbt, %w(x x)], [:sbt, [3, 'x']]])
        result = exp.expand
        expect(exp.expand).to eq expected_exp
      end
  ...
  describe '#expand_to_rsum' do
    ...
    it 'expands (((e + e)/(r + e) + (e + m))(e + r) exp into r + r + r + r' do
      ...
      exp = expression_factory.build([[nil, 2], [:add, 'x'], [:div, [[nil, r_1],
                                                                     [:add, 'y']]]])
      r_conf_1 = [[2, 'b'], [[nil, [4]], [:add, %w(y b)], [:add, %w(y a)]]]
      r_conf_2 = [%w(x b), [[nil, [4]], [:add, %w(y b)], [:add, %w(y a)]]]
      r_conf_3 = [[2, 'a'], [[nil, [4]], [:add, %w(y b)], [:add, %w(y a)]]]
      r_conf_4 = [%w(x a), [[nil, [4]], [:add, %w(y b)], [:add, %w(y a)]]]
      r_sum_conf = [[nil, r_conf_1], [:add, r_conf_2],
    ...
    it 'expands some crazy stuff correctly!' do
      ...
      exp = expression_factory.build([[nil, 2], [:add, 'x'],
                                      ...
                                      [:add, [[nil, 'c'], [:add, 5]]],
                                      [:mtp, [[nil, 6], [:add, r_3]]]])
      r_conf_1 = [[2, 'b', 'e', 'e', 6], [[nil, [4, 'f', 'e']], [:add, %w(y f b e)]]]
      r_conf_2 = [['x', 'b', 'e', 'e', 6], [[nil, [4, 'f', 'e']], [:add, %w(y f b e)]]]
      r_conf_3 = [['c', 6], [[nil, [1]]]]
      r_conf_4 = [[5, 6], [[nil, [1]]]]
      r_conf_5 = [[2, 'b', 'e', 'e', 'p'], [[nil, [4, 'f', 'e', 'q']], [:add, %w(y f b e q)]]]
      r_conf_6 = [%w(x b e e p), [[nil, [4, 'f', 'e', 'q']], [:add, %w(y f b e q)]]]
      r_conf_7 = [%w(c p), [[nil, ['q']]]]
      r_conf_8 = [[5, 'p'], [[nil, ['q']]]]
    ...
    it 'expands (((e - e)/(r - e) exp into r - r - r + r' do
      ...
      exp = expression_factory.build([[nil, 2], [:sbt, 'x'], [:div, [[nil, r_1],
                                                                     [:sbt, 'y']]]])
      r_conf_1 = [[2, 'b'], [[nil, [4]], [:sbt, %w(y b)], [:add, %w(y a)]]]
      r_conf_2 = [%w(x b), [[nil, [4]], [:sbt, %w(y b)], [:add, %w(y a)]]]
      r_conf_3 = [[2, 'a'], [[nil, [4]], [:sbt, %w(y b)], [:add, %w(y a)]]]
      r_conf_4 = [%w(x a), [[nil, [4]], [:sbt, %w(y b)], [:add, %w(y a)]]]
      r_sum_conf = [[nil, r_conf_1], [:sbt, r_conf_2],
  ...
  describe '#expand_with_details' do
    ...
    context '#_expand_wd_mtp_step' do
      it 'expands a mtp step into the current result arrays' do
        ...
        expansion_details = %w(stage_1 stage_2)
        exp = expression_factory.build([])
