...
class Fraction
  ...
  def self.random(parameters={})
    ...
    new_fraction = self.new(integer,numerator,denominator).simplify
    if new_fraction.numerator == 0
      return self.random(parameters)
    else
      return new_fraction
    end
  end
  ...
  def self.generate_question(parameters={})
    ...
    return self._generate_addition(parameters) if operation == :add
    return self._generate_subtraction(parameters) if operation == :sbt
    return self._generate_multiplication(parameters) if operation == :mtp
    return self._generate_division(parameters) if operation == :div
    nil
  end
  ...
  def self._generate_addition(parameters)
    fraction_1 = self.random(parameters)
    fraction_2 = self.random(parameters)
  ...
  def self._generate_subtraction(parameters)
    fraction_1 = self.random(parameters)
    fraction_2 = self.random(parameters)
    if fraction_2 > fraction_1
      fraction_1,fraction_2 = fraction_2,fraction_1
    end
