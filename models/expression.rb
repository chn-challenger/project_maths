...
class Expression
  ...
  def _all_steps_numerical?
    steps.each do |step|
      return false if !step.val.is_a?(Integer) && !step.val.is_a?(Float)
    end
    true
  end
  ...
  def expand
    ...
    steps.first.ops = nil # this is to be taken out once nullify first step is written
    self
  end
  ...
  def _need_brackets?
    ...
    steps.last.ops == :mtp || steps.last.ops == :div ? false : true
  end
  ...
  def _clean_nrator
    ...
    steps << _nil_one_step if steps.empty?
    steps.first.ops = nil
  end
  ...
  def _clean_dnator
    steps.each do |d_step|
      ...
      d_step.val.steps << _nil_one_step if d_step.val.steps.empty?
  ...
  def _bsort_m_form_steps
    ...
    self == copy ? self : _bsort_m_form_steps
  end
  ...
  def simplify_a_m_sum
    ...
    until m_form_steps.empty?
      ...
      result_steps += combined_step_array
    end
    self.steps = result_steps
    unless steps.empty?
      steps.first.val.steps.first.val *= -1 if steps.first.ops == :sbt
      steps.first.ops = nil
    end
    self
  end
  ...
  def _wrap_into_mforms(steps)
    steps.inject([]) do |m_steps, step|
      m_steps << if step.exp_valued?
                   step
                 else
                   step_factory.build([step.ops, [[nil, step.val]]])
                 end
    end
  end
  ...
  def simplify_all_m_sums
    return simplify_a_m_sum if is_m_form_sum?
    steps.each do |step|
      next unless step.exp_valued?
  ...
  def _expand_wd_part_1
    ...
    step_stage_arrays = []
    steps.each do |step|
      step_stage_arrays << if step.exp_valued?
                             step.val.expand_with_details
                           else
                             [step.to_msum.val]
                           end
    end
  ...
  def _expand_wd_part_2(step_stage_arrays)
    ...
    for i in 0...num_of_stages
      ...
      for j in 0...steps.length
        ...
        curr_step_val = if i >= step_stage_arrays[j].length
                          step_stage_arrays[j].last
                        else
                          step_stage_arrays[j][i]
                        end
  ...
  def _expand_wd_add_sbt_streak(expanded_steps, add_sbt_step_streak, remaining_steps, expansion_details)
    ...
    expansion_details << simplified_stage if current_stage != simplified_stage
