...
class LatexPrinter
  FUTURE_NOTES = "\\usepackage{lastpage}\\pagestyle{fancy}  and get rid of \\pagenumbering{gobble}  also add  \\cfoot{\\thepage\ of \\pageref{LastPage}}  to get page numbers".freeze
  ...
  def self.worksheet_ends(topic, sheet_number, student)
    title = topics[topic][:work_sheet_title]
    topic_prefix = topics[topic][:prefix]
    result = { questions: {}, solutions: {} }
    serial = generate_serial
    result[:questions][:start] = HEADERS
    result[:questions][:start] += "\\lfoot{#{topic_prefix}-#{serial}-Q\\quad \\textc"\
  ...
  def self.paper(contents, paper_number, student, layout = {})
    ...
    worksheet_ends = paper_ends(paper_number, student)
  ...
  def self.worksheet(topic, sheet_number, student, number_of_questions, parameters = {}, layout_format = {})
    ...
    content_latex = worksheet_content_latex(questions, topic, layout_format)
  ...
  def self.rails_sheet(topic, number_of_questions, parameters = {})
    ...
    content_latex = rails_sheet_latex(questions, topic, parameters)
